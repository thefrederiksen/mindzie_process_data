...
## Quick Start: 3-Phase Process

### PHASE 1: Process Definition & Setup (Required)
**Goal**: Define the process and prepare the environment

1. **Create directory structure** - Use the standard template below
2. **Create README.md** - Basic project description
3. **Create process_specifications.md** - Complete process definition including:
   - Activities (10-15 recommended)
   - Case attributes
   - Process flows and decision points
   - Problems/bottlenecks to inject
   - Time scales and thresholds
4. **Create activities.json** - Activity definitions with ownership
5. **Create requirements.txt** - Python dependencies
6. **Create .env.example** - Template for API credentials
7. **Copy mindzie_swagger.json** - API specification from existing project

### PHASE 2: Historical Data & Analysis (Required)
**Goal**: Generate and analyze 1-2 years of completed process data

8. **Create process_historical.md** - Historical analysis approach
9. **Implement historical_event_log.py** - Generate historical data
10. **Create historical_event_log_stats.py** - Statistical validation script
11. **Run statistics & validate** - Verify data matches specifications
12. **Review statistics with agent** - Identify missing problems or unrealistic patterns
13. **Iterate data generation** - Fix issues found in validation
14. **Implement historical_dataset_upload.py** - Upload to Mindzie
15. **Test upload** - Verify dataset in Mindzie Studio
16. **Create process_historical_analysis.md** - Analysis specifications
17. **Create Mindzie project (.mpz)** - Build dashboards and analyses
18. **Document findings** - Update README with insights

### PHASE 3: Real-Time Monitoring (Optional)
**Goal**: Enable command center style monitoring of open/active cases

19. **Create process_current_day.md** - Real-time monitoring approach
20. **Implement daily_event_log.py** - Generate current state snapshot
21. **Create daily_event_log_stats.py** - Validate open cases
22. **Run statistics & validate** - Check case distribution is realistic
23. **Review statistics with agent** - Ensure proper bottlenecks visible
24. **Iterate data generation** - Adjust open case logic
25. **Implement daily_dataset_upload.py** - Upload current state
26. **Create MCL files** - Define monitoring stages
27. **Configure enrichments** - Set up calculations
28. **Build monitoring dashboards** - Command center views
29. **Test real-time updates** - Verify refresh cycles
30. **Create operational guides** - How to use monitoring

---

## Overview
This document provides detailed instructions for creating new sample process mining datasets. Follow the Quick Start guide above for the basic process, then refer to the sections below for detailed requirements and best practices.
...
- **`process_specifications.md`** - Complete process specification including:
  - Activities tracked (10-15 activities recommended)
...
#### JSON Format (Primary - Case-Centric Structure)
The JSON format must organize data by CaseId with activities as a nested array. This structure:
- Makes it easier to view complete case journeys
- Provides better readability for analysis
- Serves as the source for CSV generation
      ...
      "activities": [
        {
          ...
          "ActivityTime": "2024-01-15T09:00:00Z",
          "Resource": "Resource Name",
          // Activity-specific attributes
        },
        {
          "ActivityName": "Activity 2",
          "ActivityTime": "2024-01-15T10:00:00Z",
          "Resource": "Resource Name",
          // Activity-specific attributes
        }
      ],
      // Case-level attributes (constant for all activities in the case)
      "Attribute1": "Value1",
      "Attribute2": "Value2"
    }
  ]
...
#### CSV Format (Generated from JSON)
The CSV format is generated by flattening the JSON structure:
- One row per activity
- Case-level attributes repeated for each activity
- All timestamps in ISO 8601 format

**Generation Order:**
1. **Always generate JSON first** with the case-centric structure
2. **Then generate CSV** by flattening the JSON data
3. This ensures consistency between formats
